return {
    ...
    config = function()
        -- Guard with pcall since hot.nvim is optional
        local ok_hot, hot_params = pcall(require, "hot.params")
        if not ok_hot or not hot_params then
            return
        end

        -- Enable Current directory
        vim.opt.autochdir = true
        local opts = hot_params.opts
